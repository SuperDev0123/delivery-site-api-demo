...
def mapBokToBooking(request):
    ...
    try:
        ...
        if not option_value.is_running or run_time > 0:
            ...
            option_value.save()
            max_id = Bookings.objects.filter().aggregate(max_id=Max('b_bookingID_Visual')).get('max_id')
            ...
            for booking in error_bookings:
                Bookings.objects.filter(id=booking["id"]).update(b_error_Capture=sliceString(('SINV is duplicated in bookingID = ' + booking['b_bookingID_Visual_List']), 1000), b_status='On Hold')
    ...
    try:
        with transaction.atomic():
            ...
            booking = Bookings.objects.create(
                ...
                b_clientReference_RA_Numbers = sliceString(header.b_000_1_b_clientReference_RA_Numbers, 1000),
                ...
                b_handling_Instructions = sliceString(header.b_014_b_pu_handling_instructions, 120),                
                pu_PickUp_Instructions_Contact = sliceString(header.b_015_b_pu_instructions_contact, 512),                
                pu_pickup_instructions_address = sliceString(header.b_016_b_pu_instructions_address, 512),                
                ...
                pu_Address_PostalCode = sliceString(header.b_033_b_pu_address_postalcode, 25),                
                pu_Address_Country = sliceString(header.b_034_b_pu_address_country, 50),                
                pu_Contact_F_L_Name = sliceString(header.b_035_b_pu_contact_full_name, 25),                
                pu_email_Group = sliceString(header.b_036_b_pu_email_group, 512),                
                pu_Phone_Main = sliceString(header.b_038_b_pu_phone_main, 25),                
                pu_Comm_Booking_Communicate_Via = sliceString(header.b_040_b_pu_communicate_via, 25),                
                ...
                de_to_Pick_Up_Instructions_Contact = sliceString(header.b_043_b_del_instructions_contact, 512),                
                de_to_PickUp_Instructions_Address = sliceString(header.b_044_b_del_instructions_address, 512),                
                ...
                de_To_Address_Street_1 = sliceString(header.b_055_b_del_address_street_1, 40),                
                de_To_Address_Street_2 = sliceString(header.b_056_b_del_address_street_2, 40),                
                ...
                de_To_Address_PostalCode = sliceString(header.b_059_b_del_address_postalcode, 30),                
                de_To_Address_Country = sliceString(header.b_060_b_del_address_country, 12),                
                de_to_Contact_F_LName = sliceString(header.b_061_b_del_contact_full_name, 50),                
                ...
                de_To_Comm_Delivery_Communicate_Via = sliceString(header.b_066_b_del_communicate_via, 40),                
                total_1_KG_weight_override = header.total_kg,                
                zb_002_client_booking_key = sliceString(header.v_client_pk_consigment_num, 64),                
            ...
            bok_lines = BOK_2_lines.objects.filter(success__in=[2,4,5], v_client_pk_consigment_num=header.pk_header_id)
            ...
            bok_lines_data = BOK_3_lines_data.objects.filter(success__in=[2,4,5], v_client_pk_consigment_num=header.pk_header_id)
            for line_data in bok_lines_data:
                Booking_lines_data.objects.create(
                    ...
                    itemFaultDescription=sliceString(line_data.ld_004_fault_description, 200),
            ...
            if first_name == 'Bathroom':
                ...
                booking.booking_Created_For_Email = booking_created_for_email[0].email[:64] if len(booking_created_for_email) > 0 else ''
                booking.save()                    
