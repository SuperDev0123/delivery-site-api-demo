...
def build_xml(booking_ids, vx_freight_provider, one_manifest_file):
    ...
    if vx_freight_provider == "allied":
        ...
        if production:
            ...
            local_filepath_dup = (
                "/var/www/html/dme_api/static/xmls/allied_au/archive/"
        ...
        if production:
            ...
            local_filepath_dup = (
                "/var/www/html/dme_api/static/xmls/tas_au/archive/"
                + str(datetime.now().strftime("%Y_%m_%d"))
                + "/"
            )
        else:
...
def build_manifest(booking_ids, one_manifest_file, user_name):
    ...
    if production:
        ...
        local_filepath_dup = (
            "/var/www/html/dme_api/static/pdfs/tas_au/archive/"
            + str(datetime.now().strftime("%Y_%m_%d"))
            + "/"
        )
    else:
        ...
        for booking in bookings:
            ...
            for booking_line in booking_lines:
                for k in range(booking_line["e_qty"]):
                    ...
                    tbl_data = [
                        [
                            Paragraph("<font size=8>Item: </font>", styles["Normal"]),
                            Paragraph(
                                "<font size=12><b>%s of %s</b></font>" % (j, totalQty),
                                style_left,
                            ),
                        ]
                    ]
                    ...
                    tbl = Table(
                        tbl_data, colWidths=(60, 60, 60), rowHeights=10, hAlign="LEFT"
                    )
                    tbl.setStyle(
                        [
                            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
                    ...
                    tbl = Table(
                        tbl_data, colWidths=(90, 90, 220), rowHeights=10, hAlign="LEFT"
                    )
                    tbl.setStyle(
                        [
                            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
