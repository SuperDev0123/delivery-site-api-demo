...
def get_available_surcharge_opts(booking):
    ...
    for item in lines:
        ...
        diagonals.append(
            math.sqrt(
                item.e_dimLength ** 2 + item.e_dimWidth ** 2 + item.e_dimHeight ** 2
            )
            * _get_dim_amount(item.e_dimUOM)
        )
    ...
    to_be_considered = {
        ...
        "is_tail_lift": booking.b_booking_tail_lift_pickup
        or booking.b_booking_tail_lift_deliver,
    }
    ...
    return {"fp_name": booking.vx_freight_provider, "surcharges": applicable_surcharges}
