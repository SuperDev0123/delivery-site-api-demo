...
def build_label(
    ...
    fp_id = Fp_freight_providers.objects.get(fp_company_name="DXT").id
    try:
    ...
    logger.info(f"#111 [DXT LABEL] File full path: {filepath}/{filename}")
    ...
    dxt_logo = "./static/assets/logos/allied.png"
    dxt_img = Image(dxt_logo, 30 * mm, 7.7 * mm)

    fp_color_code = (
        Fp_freight_providers.objects.get(fp_company_name="DXT").hex_color_code
        or "808080"
    )
    ...
    for booking_line in lines:
        for k in range(booking_line.e_qty):
            ...
            tbl_parcelId = [
                [
                    Paragraph(
                        "<font size=%s>&nbsp;&nbsp; Item No: <b>DME%s%s</b></font>"
                        % (
                            label_settings["font_size_medium"],
            ...
            to_del_data.append(
                [
                    ...
                    Paragraph(
                        "<font size=%s><b>%s</b></font>"
                        % (label_settings['font_size_large'], booking.de_to_Phone_Main[:30]),
                        style_left_bg,
                    ),
                    "",
                ]
            ...
            tbl_data1 = [
                [
                    Paragraph(
                        ...
                        % (
                            label_settings["font_size_medium"],
                            ((booking.de_to_PickUp_Instructions_Address or "") + " " + (booking.de_to_Pick_Up_Instructions_Contact or ""))[:70],
                        ),
                        style_left,
                    )
                ],
